...
def test_update_budget_preference(orchestrator, test_user_id):
    ...
    tool_calls = [tr for tr in result.get("tool_results", []) if tr["tool"] == "update_user_preferences"]
    
    if tool_calls:
        # If tool was called, verify it worked correctly
        tool_result = tool_calls[0]["result"]
        assert "error" not in tool_result, f"Tool execution error: {tool_result.get('error')}"
        assert tool_result.get("budget_max") == 30, f"Expected budget_max=30, got {tool_result.get('budget_max')}"
        ...
        assert len(result.get("response", "")) > 0, "Should have a response even if tool wasn't called"
...
def test_conversation_sequence(orchestrator, test_user_id):
    ...
    tool_calls1 = [tr for tr in result1.get("tool_results", []) if tr["tool"] == "update_user_preferences"]
    ...
    # Check for tool calls
    tool_calls2 = [tr for tr in result2.get("tool_results", []) if tr["tool"] == "update_user_preferences"]
    
    # Verify conversation history is maintained
    assert len(orchestrator.conversation_history) > 4, "Conversation history should contain multiple messages"
    
    # Check that conversation history has proper structure
    user_messages = [msg for msg in orchestrator.conversation_history if msg.get("role") == "user"]
    assert len(user_messages) == 2, "Should have 2 user messages in conversation history"
    
    assistant_messages = [msg for msg in orchestrator.conversation_history if msg.get("role") == "assistant"]
