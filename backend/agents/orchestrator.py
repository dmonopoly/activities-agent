...
from agents.mock_data import MOCK_RESPONSES_WITH_TOOLS, make_mock_completion
from agents.tools.google_maps import (
    TOOL_DEFINITION as GOOGLE_MAPS_TOOL,
)
from agents.tools.google_maps import (
...
from agents.tools.scraper import TOOL_DEFINITION as SCRAPER_TOOL
from agents.tools.scraper import scrape_activities
from agents.tools.sheets import TOOL_DEFINITION as SHEETS_TOOL
from agents.tools.sheets import save_to_sheets
from agents.tools.weather import (
    TOOL_DEFINITION as WEATHER_TOOL,
)
from agents.tools.weather import (
...
class AgentOrchestrator:
    ...
    def _get_completion(self, model: str) -> ChatCompletion:
        ...
        if not ENABLE_OPENROUTER_API:
            ...
            if self._mock_iteration == 1 and tool_calls:
                ...
                print("[ORCHESTRATOR] ❌ API DISABLED - returning MOCK tool calls")
                ...
                print("[ORCHESTRATOR] ❌ API DISABLED - returning MOCK final response")
                print(f"[ORCHESTRATOR] Mock response preview: {content[:80]}...")
                return make_mock_completion(content=content)

        print(
            f"[ORCHESTRATOR] ✅ API ENABLED - calling OpenRouter API with model={model}"
        )
        try:
            ...
            if (
                ...
                print(
                    "[ORCHESTRATOR] ❌ RATE LIMIT ERROR - Retrying with backup model"
                )
    ...
    def _execute_tool(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        ...
        if tool_def:
            required_params = (
                tool_def.get("function", {}).get("parameters", {}).get("required", [])
            )
    ...
    def _build_skipped_tools_message(self, skipped_tools: list[str]) -> str:
        ...
        if not skipped_tools:
            return None
        tools_str = (
            " and ".join(skipped_tools)
            if len(skipped_tools) <= 2
            else ", ".join(skipped_tools[:-1]) + f", and {skipped_tools[-1]}"
        )
        return f"I wanted to use {tools_str}, but the website owner has disabled them unless you know the magic word."

    def process_message(
        self, message: str, model: str = OPENROUTER_DEFAULT_MODEL
    ) -> dict[str, Any]:
        """
        ...
        print(
            f"[ORCHESTRATOR] Conversation history length: {len(self.conversation_history)}"
        )
        ...
        while iteration < max_iterations:
            ...
            if not response.choices:
                print("[ORCHESTRATOR] ❌ ERROR: Empty choices in API response")
                return {
            ...
            tool_call_names = [
                tc.function.name or "(function.name missing)"
                for tc in (message_response.tool_calls or [])
            ]
            print(
                f"[ORCHESTRATOR] Response received: tool_call_names={tool_call_names}, content_len={len(final_message_content)}"
            )
            ...
            if message_response.tool_calls:
                filtered_tools, skipped_tools = filter_to_available_tools(
            ...
            for tool_call in assistant_message["tool_calls"]:
                ...
                print(
                    f"[ORCHESTRATOR] Executing tool: {tool_name} with args: {arguments}"
                )
        ...
        if iteration == max_iterations:
            print(
                f"[ORCHESTRATOR] Max iterations reached ({iteration}/{max_iterations}), generating final summary response"
            )
        ...
        if all_skipped_tools:
            ...
            if summary_response and summary_response.choices:
                final_message_content = (
                    (summary_response.choices[0].message.content or "") + postscript
                ).strip()
                self.conversation_history.append(
                    {"role": "assistant", "content": final_message_content}
                )
        return {
            "response": final_message_content
            or "I found some results but couldn't summarize them.",
            "tool_results": all_tool_results,
